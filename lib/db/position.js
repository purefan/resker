...
const debug = require('debug')('resker-app:lib:db:position')
...
class Position {
     ...
     * @param {String} analysis.client - The client who is adding this analysis
    ...
    async add_eval(analysis) {
        ...
        clean_analysis.client = this.client
        if (!this.fen) {
        ...
        log('Update params: %s', JSON.stringify(clean_analysis).substr(0, 100))
    ...
    async set_status(new_value) {
        ...
        const set_params = {
            status: parseInt(new_value)
        }

        if (this.client) {
            set_params.client = this.client
        }

        log('Updating %s to status %s', this.fen, set_params)
     ...
     * @param {Number} [position.multipv_goal=4] - How many variations we want
        ...
        if (existing_one) {
            ...
            const update_params = {
                $set: {
                    ...
                    multipv_goal: Math.max(position.multipv_goal || 4, existing_one.multipv_goal),
                    status: 0, // reset so it gets picked up again
            ...
            const insert_query = {
                $setOnInsert: {
                    ...
                    multipv_goal: insert_params.multipv_goal || 4,
                    client: insert_params.client,
                    created: Date.now()
                }
            }
